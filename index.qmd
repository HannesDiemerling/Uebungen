...
## Interaktives Tool für die präzisionsbasierte Planung von Mindeststichprobengrößen {.unnumbered}

Wie groß muss eine Stichprobe sein, damit ein Parameter mit der gewünschten Präzision geschätzt werden kann? Dieses Tool unterstützt die präzisionsbasierte Stichprobenplanung für verschiedene Studiendesigns. Auf Basis von Konfidenzniveau, Designparametern und tolerierbarer Margin of Error (MOE) wird die erforderliche Mindeststichprobengröße $N$ berechnet und der Zusammenhang zwischen Präzision und Stichprobenumfang anschaulich dargestellt.
...
HINWEISE = {
    ...
    "reg_simple":
        ...
        "näherungsweise durch die Varianz des Kriteriums selbst zu ersetzen, also dessen "
        "quadrierte vermutete Standardabweichung als Ausgangspunkt zu verwenden. (Man nimmt damit "
...
def n_reg_simple(z, sigma_e, moe, var_x):        return 1 + (z ** 2) * sigma_e / ((moe ** 2) * var_x)
def n_reg_mult_b(z, sigma_e, moe, var_x, vif):   return 1 + (z ** 2) * sigma_e * vif / ((moe ** 2) * var_x)
...
def n_reg_mw(z, sigma_e, m, icc, moe, var_t, var_g):
    return ((z ** 2) * sigma_e * (1 + (m - 1) * icc)) / (m * (moe ** 2) * var_t * var_g)
def n_ml_beta(z, sigma_e, m, icc, moe):
    return ((z ** 2) * sigma_e * (1 + (m - 1) * icc)) / (m * (moe ** 2))
...
inputs_reg_simple = ui.div(
    ui.input_numeric("rs_sigma_e", "Residualvarianz sigma_e_Quadrat", value=4.0,  min=0.0001, step=0.1),
...
inputs_reg_mult_b = ui.div(
    ui.input_numeric("rmb_sigma_e", "Residualvarianz sigma_e_Quadrat", value=4.0,  min=0.0001, step=0.1),
...
inputs_reg_mw = ui.div(
    ui.input_numeric("rmw_sigma_e", "Residualvarianz sigma_e_Quadrat", value=4.0, min=0.0001, step=0.1),
...
inputs_ml_b = ui.div(
    ui.input_numeric("mlb_sigma_e", "Residualvarianz sigma_e_Quadrat", value=4.0, min=0.0001, step=0.1),
...
inputs_ml_beta = ui.div(
    ui.input_numeric("mlbeta_sigma_e", "Residualvarianz sigma_e_Quadrat (Standard: 1)", value=1.0, min=0.0001, step=0.1),
...
> Diemerling, H., & Schäfer, T. (Jahr des Abrufs). *Stichprobenrechner* [Statistische Software]. https://www.stichprobenrechner.app
